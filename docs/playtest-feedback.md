...
## 2026-09-14 — GGrub Card Slug Effect
- What happened: I played the heal 1 heart effect/reward from defeating the sluf and received the following error: "Function setDoc() called with invalid data. Unsupported field value: undefined (found in document sessions/9QCJ)". The heart still healed on my chicken tho
- Expected: 
- Card/rule involved: N/A
- Triage: bug, confirmed — not Slug-specific. `playerPanel.js`'s `PlayCardControls`/`RailPlayPrompt` build the `params` object for every Bonus/Grub Card play with one key per possible input shape (`option`, `targetPlayerId`, `amount`, `discardExtraCardIndex`), explicitly set to `undefined` whenever that particular card doesn't need it — which is true for almost every card, Slug's `selfDelta: { health: 1 }` included. That dispatched action rides along in `state.actionLog`, which `remoteSession.js`'s `toSyncedState` ships to Firestore as-is, and the Firestore SDK rejects any `undefined` field value in a `setDoc` unless told otherwise. The local engine effect had already applied by the time the sync write threw, which is why the heal landed but the error still fired.
- Status: fixed — `remoteSession.js` now initializes Firestore with `initializeFirestore(app, { ignoreUndefinedProperties: true })` instead of `getFirestore(app)`, so `undefined` fields are silently dropped (same as `JSON.stringify` already does) rather than rejected. One-line fix at the SDK boundary rather than chasing every params-builder call site.

## 2026-09-14 — End Turn/Cancel
- What happened: Once I click end turn, I cannot close the dossier to return to the playing board. if I accidentally click the button with actions remaining, they are wasted
- Expected: I should be able to return to the board and take actions (or excersize an extra action if the token is still available)
- Card/rule involved: N/A
- Triage: bug, confirmed — clicking End Turn as the day's last player sets `dayEndPending` true immediately (`app.js`'s `handleEndTurn`), which renders the day-end `TurnControls` dossier full-screen. It only ever had a "Confirm and Advance" button — no way back to the board, so an accidental click (or realizing you still wanted to use a remaining action first) left you stuck.
- Status: fixed — added a "Back to board" button (`handleCancelDayEnd` in `app.js`, wired to a new `onCancel` prop on `TurnControls`) that just flips `dayEndPending` back to false, locally and via `remoteSession.pushState`. Safe to do unconditionally: in this path nothing about turn/action state was ever touched when `dayEndPending` was set — `endTurn`/`advanceDay` aren't called until "Confirm and Advance," so canceling restores exactly the board the player was on, actions intact. Gated by the same `canAct` (day's-last-player) check as Confirm, so a teammate can't cancel out from under whoever's actually finishing the day.

## 2026-09-14 — Egg Exchange
- What happened: I moved from day 5 to day 6 in summer and didn't have a weather card or predator effect that restricted by ability to participate in the next egg exchange. After changing days, I still have 8 eggs and 0 food in day 6 of summer. When I end my turn I don't see an egg exchange option available for turns ending right before an egg exchange
- Expected: I should be able to exchange eggs to recieve food. This may be a better decision to use a dossier to build out a visual for egg exchanges and show new weather cards (at the table it's a more public ordeal "Ok which grub are we discarding? What's our new weather? *reaction* Egg Exchange!" type of feel)
- Card/rule involved: N/A
- Triage: bug, confirmed — the day-end Egg Exchange section itself was showing correctly on the right days (`isLastDayOfPhase` covers days 2/5/7, so day 5→6 should have offered it). The real bug: `TurnControls`'s `exchanges` amounts were plain per-device `useState`, never synced — every player's device rendered the same dossier and could type into their own row, but only whichever device actually clicked "Confirm and Advance" had its local amounts (for every player, not just itself) actually used; everyone else's typing was pure UI theater that got silently discarded. If you weren't the one who clicked Confirm, your egg amount was whatever (usually 0, unfilled) the confirming device happened to have for your row — matching "8 eggs, 0 food" exactly.
- Status: fixed — each player's amount is now synced (`remoteSession.setPendingExchange`, one key per player so no two players race on the same field) as they type, read back via the existing `onSnapshot` subscription into `app.js`'s `pendingExchanges` state. Each row is now editable only by its own player (`myPlayerId === p.id`); everyone else's row shows read-only. "Confirm and Advance" reads this synced map instead of fabricating it from local-only state. `remoteSession.clearPendingExchanges` resets it once the day actually advances. Didn't build the fuller "public reveal" dossier redesign floated in the report — that's a design question worth a separate check-in, this just fixes the actual data-loss bug.